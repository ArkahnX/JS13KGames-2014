...
(function() {
	...
	sonantx.SoundGenerator = function(instr, rowLen) {
        var self = this;
		self.instr = instr;
		self.rowLen = rowLen || 5605;

		self.osc_lfo = oscillators[instr.lfo_waveform];
		self.osc1 = oscillators[instr.osc1_waveform];
		self.osc2 = oscillators[instr.osc2_waveform];
		self.attack = instr.env_attack;
		self.sustain = instr.env_sustain;
		self.release = instr.env_release;
		self.panFreq = Math.pow(2, instr.fx_pan_freq - 8) / self.rowLen;
		self.lfoFreq = Math.pow(2, instr.lfo_freq - 8) / self.rowLen;
	};
	sonantx.SoundGenerator.prototype.genSound = function(n, chnBuf, currentpos) {
		...
		var o1t = getnotefreq(n + (self.instr.osc1_oct - 8) * 12 + self.instr.osc1_det) * (1 + 0.0008 * self.instr.osc1_detune);
		var o2t = getnotefreq(n + (self.instr.osc2_oct - 8) * 12 + self.instr.osc2_det) * (1 + 0.0008 * self.instr.osc2_detune);

		// State variable init
		var q = self.instr.fx_resonance / 255;
		var low = 0;
		var band = 0;
		for (var j = self.attack + self.sustain + self.release - 1; j >= 0; --j) {
			var k = j + currentpos;

			// LFO
			var lfor = self.osc_lfo(k * self.lfoFreq) * self.instr.lfo_amt / 512 + 0.5;

			// Envelope
			var e = 1;
			if (j < self.attack)
				e = j / self.attack;
			else if (j >= self.attack + self.sustain)
				e -= (j - self.attack - self.sustain) / self.release;

			// Oscillator 1
			var t = o1t;
			if (self.instr.lfo_osc1_freq) t += lfor;
			if (self.instr.osc1_xenv) t *= e * e;
			c1 += t;
			var rsample = self.osc1(c1) * self.instr.osc1_vol;

			// Oscillator 2
			t = o2t;
			if (self.instr.osc2_xenv) t *= e * e;
			c2 += t;
			rsample += self.osc2(c2) * self.instr.osc2_vol;

			// Noise oscillator
			if (self.instr.noise_fader) rsample += (2 * Math.random() - 1) * self.instr.noise_fader * e;

			rsample *= e / 255;

			// State variable filter
			var f = self.instr.fx_freq;
			if (self.instr.lfo_fx_freq) f *= lfor;
			f = 1.5 * Math.sin(f * 3.141592 / WAVE_SPS);
			low += f * band;
			var high = q * (rsample - band) - low;
			band += f * high;
            var test = self.instr.fx_filter;
            if(test === 1) { // Hipass
                rsample = high;
            }
            if(test === 2) { // Lopass
			...
			t = osc_sin(k * self.panFreq) * self.instr.fx_pan_amt / 512 + 0.5;
			rsample *= 39 * self.instr.env_master;
	...
	sonantx.SoundGenerator.prototype.getAudioGenerator = function(n, callBack) {
		var self = this;
        var bufferSize = (self.attack + self.sustain + self.release - 1) + (32 * self.rowLen);
	...
	sonantx.MusicGenerator.prototype.generateTrack = function(instr, mixBuf, callBack) {
		var self = this;
		genBuffer(self.waveSize, function(chnBuf) {
